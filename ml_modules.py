...
class DoubleConvBlock(nn.Module):
    def __init__(self, in_size, out_size, padding, batch_norm,
                 equalized=False, dropout=False, attention=False, max_pool=True):
        ...
        if max_pool:
            self.ds = nn.AvgPool2d(kernel_size=2)
...
class DoubleDeconvBlock(nn.Module):
    def __init__(self, in_size, out_size, padding, batch_norm,
                 equalized=False, dropout=False, attention=False, last_relu=True, max_pool=True):
        ...
        for channels in self.feature_channels:
            self.net.append(
                DoubleConvBlock(prev_channels, channels,
                                padding, batch_norm,
                                lr_equalization, dropout, attention, max_pool)
            )
...
class CNNEncoderVGG16(nn.Module):
    def __init__(self, in_channels=3, batch_norm=False, pretrained=False, flip_images=False):
        super(CNNEncoderVGG16, self).__init__()
        self.pretrained = pretrained

        if pretrained:  # gaussian_pyramid cannot be used with a pretrained VGG16
            self.in_channels = 3
        else:
            self.in_channels = in_channels
        self.feature_channels = [64, 128, 256, 1024, 2048]  # 2048
        ...
        self.net.append(
            DoubleConvBlock(prev_channels, next_channels,
                            padding=True, batch_norm=batch_norm,
...
class Unet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, config=None, gaussian_pyramid=True):
        super().__init__()
        self.gaussian_pyramid=gaussian_pyramid
        if gaussian_pyramid:
            self.gaussian_filters = GaussianPyramid(kernel_size=11, kernel_variance=1., num_octaves=config.num_octaves,
                                                    octave_scaling=1.414)
        
            in_channels=in_channels+config.num_octaves
    ...
    def mask(self, x):
        if not hasattr(self, "mask_2d"):
            length=self.config.ProjectionSize
            ax=torch.linspace(-length/2.0,length/2.0,length ).cuda()
            diameter=self.config.mask_3d_diameter
            xx,yy=torch.meshgrid(ax, ax)
            self.mask_2d=(xx**2+yy**2).sqrt()<length/2.0*diameter
        
        return x*self.mask_2d
...
class CNNProgressiveEncoder(nn.Module):
    def __init__(self, in_channels=1, feature_channels=[64,64,64],
                 padding=True, batch_norm=True, max_pool=True,
                 lr_equalization=False, dropout=False, attention=False,
                 global_avg_pool=True):
        super(CNNProgressiveEncoder,self).__init__()
        print(in_channels)
